...
5. 変換がバックグラウンドで開始されます。進行状況は通知センターに表示されます。
6. 変換完了後、自動的に Capture One のインポート画面が開くので、DNGファイルをインポートしてください。
...
### 有効にする方法
- **Lightroom**: 変換設定ダイアログで **「Normalize WhiteLevel (fixes Capture One highlight yellow)」** にチェックを入れます。
- **Capture One**: 変換中に表示される「ホワイトレベル正規化（Normalize WL）を適用しますか？」ダイアログで **「適用する (Yes)」** を選択します。

> **注意**: このオプションは主に Capture One 向けの補正です。Lightroom は per-channel ホワイトレベルを正しく処理するため本来不要ですが、有効にしても Lightroom での表示に悪影響はありません。
