...
### 1. インストール
1. [Releases ページ](https://github.com/shida-ito/X3F-Plugins/releases) から最新版をダウンロードして展開し、`Lightroom/X3FforLrC.lrplugin` を任意の場所（例: `~/Documents/Plugins/`）に保存します。
2. **Apple Silicon Mac のみ**: 同梱バイナリを実行できるようにするため、ターミナルで以下を実行してください（macOS Sequoia 以降で必須）：
   ```bash
...
4. **「追加」** ボタンをクリックし、先ほど保存した `.lrplugin` フォルダを選択します。
...
- **Lightroom（Apple Silicon）**: 上記の[Lightroom インストール手順](#1-インストール)のステップ 2 を参照してください。Apple Silicon（macOS Sequoia 以降）では、署名のない arm64 バイナリは AMFI によってブロックされ、「このまま開く」ダイアログが表示されません。`xattr` と `codesign` コマンドの実行が必要です。
